...
## 中文说明
### 服务端
* 安装本 Mod 后，服务端可接受玩家通过 WebSocket 接入。
* 若未单独禁止，玩家仍可通过原版 TCP 直接接入。
* 在原有 TCP 端口上监听 WebSocket 升级请求，端口仍由 server.properties 控制。
* 未安装本 Mod 的客户端仍可用原版 TCP 加入装有本 Mod 的服务器（其他 Mod 可能阻止加入）。
* WebSocket 握手完成后，服务端可从 HTTP 头获取客户端信息（如 `X-Forwarded-For` 获取真实 IP 等）。

### 客户端
* 安装本 Mod 后，可用 `ws://域名.com:端口/WebSocket路径` 加入同样安装了本 Mod 的服务器。
* 仍可用原版的 `域名.com:端口` 方式加入原版服务器。
